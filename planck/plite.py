...
def main():
    ...
    TTTEEE2018_lowTTbins = PlanckLitePy(
        year=2018, spectra="TTTEEE", use_low_ell_bins=True
    )
...
class PlanckLitePy:
    ...
    def test(self):
        ls, Dltt, Dlte, Dlee = np.genfromtxt(
            f"{self.data_directory}/Dl_planck2015fit.dat", unpack=True
        )
        ellmin = int(ls[0])
        ...
        if self.year == 2018 and self.spectra == "TTTEEE" and not self.use_low_ell_bins:
            ...
            self.year == 2015 and self.spectra == "TTTEEE" and not self.use_low_ell_bins
        ):
